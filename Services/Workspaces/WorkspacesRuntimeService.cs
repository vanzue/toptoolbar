...
namespace TopToolbar.Services.Workspaces
{
    internal sealed partial class WorkspacesRuntimeService : IDisposable
    {
        ...
        public async Task<bool> LaunchWorkspaceAsync(string workspaceId, CancellationToken cancellationToken)
        {
            ...
            swLoad.Stop();
            LogPerf($"WorkspaceRuntime: Loaded workspace '{workspaceId}' in {swLoad.ElapsedMilliseconds} ms");
            if (workspace == null)
            {
            ...
            foreach (var app in workspace.Applications)
            {
                ...
                try
                {
                    ...
                    LogPerf($"WorkspaceRuntime: [{app?.Id}] Ensure app alive - begin");
                    var result = await MakeSureAppAliveAsync(app, context, cancellationToken).ConfigureAwait(false);
                    swEnsure.Stop();
                    LogPerf($"WorkspaceRuntime: [{app?.Id}] Ensure app alive - done in {swEnsure.ElapsedMilliseconds} ms; success={result.Succeeded}; windows={result.Windows.Count}; launchedNew={result.LaunchedNewInstance}");
                    ...
                    swPlace.Stop();
                    LogPerf($"WorkspaceRuntime: [{app?.Id}] SetWindowPlacement took {swPlace.ElapsedMilliseconds} ms");
                    anySuccess = true;
                }
            ...
            swMinimize.Stop();
            LogPerf($"WorkspaceRuntime: MinimizeExtraneousWindows took {swMinimize.ElapsedMilliseconds} ms");

            swTotal.Stop();
            LogPerf($"WorkspaceRuntime: LaunchWorkspace('{workspaceId}') total {swTotal.ElapsedMilliseconds} ms; anySuccess={anySuccess}");
            return anySuccess;
        }
        ...
        private static void LogPerf(string message)
        {
            try
            {
                ...
                try
                {
                    var ts = DateTime.Now.ToString("HH:mm:ss.fff", System.Globalization.CultureInfo.InvariantCulture);
